...
export function DashboardLayout({ chatId }: DashboardLayoutProps) {
  const [activeTab, setActiveTab] = useState<TabType>("key-details")

  const handleNewDocument = () => {
    localStorage.removeItem("chatid")
    localStorage.removeItem(`document_${chatId}`)
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "key-details":
        return <KeyDetailsTab chatId={chatId} />
      case "analysis":
        return <AnalysisTab chatId={chatId} />
      case "assistant":
        return <AssistantTab chatId={chatId} />
      case "checklist":
        return <ChecklistTab chatId={chatId} />
      default:
        return <KeyDetailsTab chatId={chatId} />
    }
  }
